...
| V4 | Einbindung von Schriftarten (Google Fonts) | aktiv | Art. 6 I f — **angreifbar, siehe Anmerkung** |
...
| **Kategorien betroffener Personen** | Besucher von iyambae.fm, www.iyambae.fm und apps.iyambae.fm |
...
| **Offener Punkt** | Ein Widerruf ist im Programmtext vorgesehen (`widerrufeEinwilligung()`), aber an keiner Oberfläche angeschlossen — siehe HINWEISE.md Punkt 5 |
| **Fundstellen** | `assets/lib/fehlerbericht.mjs`, `deploy/nginx.conf` |

---

## V4 — Einbindung von Schriftarten (Google Fonts)

| Feld | Inhalt |
|---|---|
| **Bezeichnung** | Nachladen von Webschriften von Google-Servern |
| **Zweck** | Einheitliche Darstellung in sieben Sprachen und zwei Schriftsystemen (lateinisch, arabisch) |
| **Rechtsgrundlage** | Art. 6 Abs. 1 lit. f DSGVO. **Diese Grundlage ist angreifbar.** Das LG München I hat mit Urteil vom 20.01.2022 (Az. 3 O 17493/20) für die dynamische Einbindung von Google Fonts eine Einwilligung verlangt und Schadensersatz zugesprochen |
| **Kategorien betroffener Personen** | alle Besucher — der Abruf erfolgt automatisch beim Seitenaufbau, ohne Zutun |
| **Kategorien personenbezogener Daten** | **vollständige, ungekürzte IP-Adresse**; Zeitpunkt; User-Agent; Referrer (`https://iyambae.fm`) — jeweils erhoben durch Google, nicht durch uns |
| **Empfänger** | Google Ireland Limited / Google LLC. Kein Auftragsverarbeitungsverhältnis — wir haben keinen Vertrag mit Google, die Verbindung baut der Browser des Besuchers auf |
| **Drittlandübermittlung** | **Ja, USA.** Stütze: Angemessenheitsbeschluss der EU-Kommission zum EU-US Data Privacy Framework vom 10.07.2023, soweit der empfangende Anbieter zertifiziert ist. Der Beschluss ist Gegenstand laufender Verfahren |
| **Löschfrist** | bei uns keine — es entsteht kein Datenbestand. Die Speicherdauer bei Google entzieht sich unserer Kenntnis. Auf dem Endgerät: Zwischenspeicherung der Schriftdateien durch den Service Worker (Laufzeitspeicher, `stale-while-revalidate`) |
| **TOM** | `Referrer-Policy: strict-origin-when-cross-origin` — übermittelt wird nur die Herkunft, nicht die vollständige Adresse; der arabische Schriftsatz wird nur auf `/ar/` geladen (`Scripts/baue-sprachen.py` entfernt die Zeile aus allen anderen Seiten); Zwischenspeicherung verringert die Zahl der Abrufe nach dem ersten Besuch |
| **Empfehlung** | **Diese Tätigkeit ersatzlos streichen**, indem die Schriften selbst ausgeliefert werden. Dann entfällt der Drittlandtransfer vollständig, und dieser Eintrag entfällt mit ihm. Ausführlich in `datenschutz.md`, Abschnitt 4, Kasten „Empfehlung an den Betreiber" |
| **Fundstellen** | `index.html` Zeilen 11–13 und 20; `sw.js` (`istSchriftart`) |
...
| **Empfänger** | [E-Mail-Anbieter des Betreibers — **einzutragen**, siehe HINWEISE.md Punkt 13]; Auftragsverarbeitungsvertrag mit diesem Anbieter erforderlich |
...
**Kein Auftragsverarbeitungsverhältnis** besteht zu Google (Schriftarten, V4) und
zu den Senderbetreibern (V5). In beiden Fällen baut der Browser des Besuchers die
Verbindung selbst auf; ein Vertrag existiert nicht.
...
- Kein geteilter Zugangsschlüssel in der Datenhaltung (geplant); Zugriff nur über
  verwaltete Identität und Azure-Rollen.
- Rollentrennung: Anwendung schreibt, Menschen lesen. Zugriff auf die
  Betriebsauswertung nur mit der Rolle „Monitoring Reader".
- Zwei Personen mit Zugriff — nicht mehr. Jede weitere Person ist eine
  Entscheidung, kein Nebeneffekt.

**Datenminimierung**

- IP-Adressen werden vor dem Schreiben gekürzt.
- Der Abfrageteil der Adresse wird weder protokolliert noch in Fehlerberichten
  übertragen.
...
- Wiederholbare Infrastruktur als Quelltext (Bicep); Änderungen sind
...
- **Keine Content-Security-Policy.** Vor der Einführung von Konten zwingend
  nachzuholen; im Quelltext bereits vermerkt.
- **Kein dokumentiertes Vorgehen für Datenschutzverletzungen** (Art. 33, 34
  DSGVO): 72 Stunden sind kurz, wenn man erst dann anfängt zu überlegen.
- **Kein Löschkonzept für inaktive Konten** (geplant, Frist offen).
- **Keine dokumentierte Prüfung der Wirksamkeit** nach Art. 32 Abs. 1 lit. d
  DSGVO. Für einen Betrieb dieser Größe genügt eine jährliche Durchsicht dieses
  Verzeichnisses, sie muss aber stattfinden und datiert werden.
